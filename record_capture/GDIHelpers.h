    ...
    namespace Screen_Capture {
        ...
        inline WindowDimensions GetWindowRect(HWND hwnd) {
            ...
            if (SUCCEEDED(DwmGetWindowAttribute(hwnd, DWMWA_EXTENDED_FRAME_BOUNDS, &frame, sizeof(frame)))) {
				//If GetWindowRect failed on dpi aware. should assign frame to clientrect.
				if (ret.ClientRect.left > frame.left || ret.ClientRect.right < frame.right || ret.ClientRect.bottom < frame.bottom)
					ret.ClientRect = frame;
